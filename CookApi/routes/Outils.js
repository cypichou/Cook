...
router.post('/', async (req, res) => {
    try{
        ...
        res.json({ message: 'Outil ajouté avec succés' });
...
router.get('/recettes/', async (req, res) => {
    try{
        ...
        const [rows] = await db.query("select outils.nom from recette_outils as ro left join outils on outils.id_outil = ro.id_outil where ro.id_recette = (?)", [id_recette])
